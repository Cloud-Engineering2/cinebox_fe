...
const SeatSelection = ({ selectedScreenId, selectedDate, selectedTime, selectedEndTime, onPayment, price, formatDayOfWeek,
    ...
    // 동적으로 rows와 seatGroups 계산하기
    const rows = [...new Set(seats.map(seat => seat.seatNumber.charAt(0)))]; // 각 좌석 번호의 첫 문자로 행 구분 (예: A, B, C...)
    const maxSeatsInRow = Math.max(...seats.map(seat => Number(seat.seatNumber.slice(1)))); // 각 행에 몇 개의 좌석이 있는지 계산

    // 좌석 그룹 계산 (여기서는 예시로 4, 10개 구분으로 설정)
    const seatGroups = rows.map(row => {
        const seatsInRow = seats.filter(seat => seat.seatNumber.charAt(0) === row);
        const groups = [];
        let group = [];
        let groupStart = 1;

        for (let i = 0; i < seatsInRow.length; i++) {
            group.push(seatsInRow[i].seatNumber);
            if ((i + 1) % 4 === 0 || i === seatsInRow.length - 1) {
                groups.push({ row, range: [groupStart, i + 1], seats: group });
                group = [];
                groupStart = i + 2;
            }
        }
        ...
        return groups;
    }).flat(); // flat을 사용해 2차원 배열을 평평하게 만듦
                ...
                <div className="seats">
                    {seatGroups.map((group, groupIndex) => (
                        <div key={groupIndex} className="row">
                            <span className="row-label">{group.row}</span>
                            {group.seats.map((seatNumber) => {
                                const seat = seats.find(seat => seat.seatNumber === seatNumber);
                                const seatId = seat?.seatId;
                                return (
                                    <button
